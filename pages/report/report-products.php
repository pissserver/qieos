<?php
error_reporting(0);
ob_start();
include '../../html2pdf/html2pdf.class.php';
include '../../script/connection.php';

$category = $_POST['category'];
$start_date = $_POST['start_date'];
$end_date = $_POST['end_date'];

if ($category == 'semua') {
    $query = mysqli_query($conn, "
        SELECT 
            p.product_name,
            COUNT(DISTINCT od.order_id) AS total_orders,
            SUM(od.qty) AS total_qty
        FROM order_details od
        JOIN products p ON od.product_id = p.id
        JOIN orders o ON od.order_id = o.id
        WHERE DATE(o.tanggal) BETWEEN '$start_date' AND '$end_date'
        AND o.status_payment != 'cancelled'
        GROUP BY p.product_name
        ORDER BY total_qty DESC
    ");
} else {
    $query = mysqli_query($conn, "
        SELECT 
            p.product_name,
            COUNT(DISTINCT od.order_id) AS total_orders,
            SUM(od.qty) AS total_qty
        FROM order_details od
        JOIN products p ON od.product_id = p.id
        JOIN orders o ON od.order_id = o.id
        WHERE DATE(o.tanggal) BETWEEN '$start_date' AND '$end_date'
        AND o.status_payment != 'cancelled'
        AND p.category = '$category'
        GROUP BY p.product_name
        ORDER BY total_qty DESC
    ");
}

if (!$query) {
    die(mysqli_error($conn));
}

$total_payment = 0;

ob_start();
?>

<style>
body { font-family: Arial; font-size: 11px; }
.header { text-align: center; margin-bottom: 10px; }
.print-date { text-align: right; font-size: 10px; margin-bottom: 5px; }
h3 { margin: 0; margin-top: 30px; }
h4 { margin: 3px 0 10px 0; font-weight: normal; }
table { width: 100%; border-collapse: collapse; margin-top: 8px; table-layout: fixed; }
th, td { border: 1px solid #000; padding: 5px; }
th { background-color: #f0f0f0; text-align: center; }
.center { text-align: center; }
.right { text-align: right; }
.signature { width: 100%; margin-top: 40px; }
.signature td { border: none; text-align: center; width: 50%; }
.footer { text-align: center; font-size: 10px; }
</style>

<page>

<div class="print-date">
    Dicetak: <?= date('d M Y'); ?>
</div>

<div class="header">
    <h3>LAPORAN PRODUK TERJUAL</h3>
    <h4>SISTEM PENJUALAN - CARTIFY</h4>
</div>

<p>
    Tanggal:
    <b><?= date("d M Y", strtotime($start_date)); ?> - <?= date("d M Y", strtotime($end_date)); ?></b>
</p>
<p style="margin-top: -5px;">
    Kategori:
    <b><?= ucfirst($category); ?></b>
</p>

<table>
<tr>
    <th width="10%">No</th>
    <th width="50%">Nama Produk</th>
    <th width="20%">Jumlah Transaksi</th>
    <th width="20%">Jumlah Qty</th>
</tr>

<?php
$no = 1;
$line = 0;

while ($row = mysqli_fetch_assoc($query)) {
    $total_transcations += $row['total_orders'];
    $total_qty += $row['total_qty'];
?>
<tr>
    <td class="center"><?= $no++; ?></td>
    <td><?= $row['product_name']; ?></td>
    <td class="center"><?= $row['total_orders']; ?> Transaksi</td>
    <td class="center"><?= $row['total_qty']; ?> Qty</td>
</tr>
<?php } ?>

<tr>
    <td colspan="2" class="center"><strong>TOTAL</strong></td>
    <td class="center">
        <strong><?= $total_transcations; ?> Transaksi</strong>
    </td>
    <td class="center">
        <strong><?= $total_qty; ?> Qty</strong>
    </td>
</tr>
</table>

<page_footer>
    <div class="footer">
        Halaman [[page_cu]] / [[page_nb]]
    </div>
</page_footer>

</page>

<?php
$html = ob_get_clean();

try {
    $pdf = new HTML2PDF('P', 'A4', 'en');
    $pdf->setDefaultFont('Arial');
    $pdf->writeHTML($html);
    $pdf->Output('laporan_omzet_bulanan.pdf', 'I');
} catch (HTML2PDF_exception $e) {
    echo $e;
}
exit;
